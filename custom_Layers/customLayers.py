...
class BatchNorm2D(Layer):
    def __init__(self, out_c, epsilon=1e-10, name=''):
        ...
        self.weight = self.add_weight(name="weights",
                                      shape=[out_c],
                                      dtype=tf.float32,
    ...
    def call(self, inputs):
        x = inputs
        x_pow = math_ops.pow(x, 2)
        x_sum = math_ops.reduce_sum(x_pow, [3], keepdims=True)
        x_sqrt = math_ops.sqrt(x_sum)
        norm = math_ops.add(x_sqrt, self.epsilon)
        inputs /= norm
        weights = tf.expand_dims(tf.expand_dims(tf.expand_dims(self.weight, 0), 0), 0)
        bias =  tf.expand_dims(tf.expand_dims(tf.expand_dims(self.bias, 0), 0), 0)
        bias = tf.tile(bias, [1, inputs.get_shape()[2], inputs.get_shape()[2], 1])
        weights = tf.tile(weights, [1, inputs.get_shape()[2], inputs.get_shape()[2], 1])
        inputs = weights * inputs + bias
        return inputs
